...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, 'generate_reports.log'),
    level=logging.DEBUG,
...
def fetch_report_ids(limit=10):
    ...
    logging.info("📥 Fetching report IDs from InsightVM.")
    reports = []
    page = 0
    ...
    try:
        ...
        logging.info(f"Fetched {len(reports)} reports.")
        return reports

    except Exception as e:
        logging.error(f"❌ Failed to fetch report IDs: {e}")
        raise
...
def trigger_report_generation(report_id):
    ...
    logging.info(f"🚀 Triggering report generation for Report ID: {report_id}")
    ...
    try:
        ...
        logging.info(f"✅ Report generation triggered for Report ID: {report_id}")
        print(f"✅ Report generation triggered for Report ID: {report_id}")
    except Exception as e:
        logging.error(f"❌ Failed to trigger report generation for Report ID: {report_id}: {e}")
...
def show_reports(limit=10):
    ...
    try:
        ...
        logging.error(f"❌ Failed to show reports: {e}")
