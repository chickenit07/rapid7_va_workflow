...
def parse_xml_for_solutions_and_products(xml_file, vuln_ids):
    ...
    for node in root.xpath(".//node"):
        ...
        if risk_score:
            try:
                # Round the risk score to make it human-readable
                rounded_risk = round(float(risk_score))
                ip_risk_map[ip_address] = rounded_risk
...
def process_vulnerabilities(df_sorted, solutions, ip_product_map, ip_os_family_map, ip_hostname_map, ip_office_product_map, ip_sharepoint_product_map, ip_exchange_product_map, ip_risk_map):
    ...
    for _, row in df_sorted.iterrows():
        ...
        if ip_address not in consolidated_vulns_by_ip:
            consolidated_vulns_by_ip[ip_address] = {"chrome": [], "firefox": [], "apache": [], "jre": [], "edge": [], "office_obsolete": [], "tomcat": [], "winrar": []}
        ...
        elif vuln_id.startswith("apache-tomcat"):
            consolidated_vulns_by_ip[ip_address]["tomcat"].append(vuln_id)
        elif vuln_id.startswith("rarlab-winrar"):
            consolidated_vulns_by_ip[ip_address]["winrar"].append(vuln_id)
    ...
    for _, row in tqdm(df_sorted.iterrows(), total=df_sorted.shape[0], desc="Processing Rows"):
        ...
        if risk_score:
            ip_with_details += f" (Risk: {risk_score})"
        ...
        if os_family.lower() == "windows":
            ...
            # Check for Apache Tomcat vulnerabilities
            if vuln_id.startswith("apache-tomcat") and consolidated_vulns_by_ip[ip_address]["tomcat"]:
                if "Outdated Apache Tomcat" not in processed_solutions_by_ip[ip_address]:
                    tomcat_solution = "Outdated Apache Tomcat => Download and apply the upgrade appropriated version from: http://archive.apache.org/dist/tomcat/"
                    output_data_windows.append([ip_with_details, tomcat_solution])
                    processed_solutions_by_ip[ip_address].add("Outdated Apache Tomcat")
                    for tomcat_vuln in consolidated_vulns_by_ip[ip_address]["tomcat"]:
                        processed_vuln_ids.add((ip_address, tomcat_vuln))
                continue
            
            # Check for WinRAR vulnerabilities
            if vuln_id.startswith("rarlab-winrar") and consolidated_vulns_by_ip[ip_address]["winrar"]:
                if "Outdated Winrar" not in processed_solutions_by_ip[ip_address]:
                    winrar_solution = "Outdated Winrar => Upgrade Rarlab WinRAR to version latest from https://www.win-rar.com/download.html"
                    output_data_windows.append([ip_with_details, winrar_solution])
                    processed_solutions_by_ip[ip_address].add("Outdated Winrar")
                    for winrar_vuln in consolidated_vulns_by_ip[ip_address]["winrar"]:
                        processed_vuln_ids.add((ip_address, winrar_vuln))
                continue
